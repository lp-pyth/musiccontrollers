...
void loop() {
  ...
  if (accel_y > 410 && millisTime - playTime > 100) {
    if (gesture(fingers,  (const int[])   { 
            156, 487, 319, 452, 489, 477, 457, 485,  
      } )) {             //FIST -start
      for (int u = 0; u < 12; u++) {                                                                    //ALL LOOP TRACKS ON
        usbMIDI.sendControlChange(CTRL_TRACKSTART + u, 127, 5);                                         //we send a 127 track value to all tracks except the pressed one
...
void sendOutput() {
  ...
  if (mode == 0) {
     ...
     */
    if (gesture(fingers, (const int[])        { 260, 4, 127, 460, 430, 426, 435, 438, }         )) {xcontrol = 1;  //pistol
                                                                                                  ycontrol = 2;}
    else if ( gesture(fingers,  (const int[])   { 156, 487, 319, 452, 489, 477, 457, 485, }       )) {xcontrol = 3;  //fist
                                                                                                  ycontrol = 4;}
    else if ( gesture(fingers,  (const int[])   { 16, 0, -12, 5, 2, -1, -1, -10, }              ))  {send_note = true;  //flat
                                                                                                  ycontrol = 5;}
    else if ( gesture(fingers,  (const int[])   { 131, 342, 15, 438, 48, 443, 26, 132, }        )) {xcontrol = 7;  //tiger
                                                                                                  ycontrol = 8;}
                                                                                                  ...
                                                                                                  ycontrol = 12;}
    else if ( gesture(fingers,  (const int[])   { 605, 13, 561, 33, 359, 463, 214, 424, }     )) {xcontrol = 13;  //3 fingers up
                                                                                                  ycontrol = 14;}
    else if ( gesture(fingers,  (const int[])   { 211, -8, 39, -7, 216, 417, 278, 491, }    )) {xcontrol = 15;  // gun
                                                                                                  ycontrol = 16;}
